...
if not model_paths:
    print(f"outputs klasöründe hiç checkpoint yok. Önce train.py'yi çalıştırmalısın.")
    exit()

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

for model_path in model_paths:
    print(f"\nSon 40'lıktan tahmin için yükleniyor: {model_path}")
    ...
    with torch.no_grad():
        ...
        # Sadece target (Close) için scaler inverse transform uygulanıyor:
        ...
        input_prices = close[-11-30:-11]

        mse = np.mean((np.array(predicted_prices) - np.array(real_future_prices))**2)
        rmse = np.sqrt(mse)

        plt.figure(figsize=(10,5))
        plt.plot(range(30), input_prices, label="Input (son 40-10 arası)")
        ...
        plt.title(f"Son 40 gün penceresiyle Tahmin\nMSE: {mse:.2f} | RMSE: {rmse:.2f}\nModel: {os.path.basename(model_path)}")
